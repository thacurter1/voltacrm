...
const getJwtSecret = (): string => {
  ...
  if (!secret) {
    if (process.env.NODE_ENV === 'production' || process.env.VOLTA_DEMO_MODE !== 'true') {
      throw new Error('FATAL SECURITY ERROR: JWT_SECRET è obbligatoria in produzione o fuori dalla modalità demo.');
    }
